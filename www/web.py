...
# setting up database
app.config["SQLALCHEMY_DATABASE_URI"] = 'mysql+mysqlconnector://root:rootroot@localhost:3306/eventweb'  # configure URI
...
def home():  # home page function
    ...
    return render_template("home.html", username=username, success=request.args.get("success"),
                           info="You can look around the website from here.")  # load home page with information
    ...
    if request.method == "GET":  # if user loads the page
        ...
        if username != None:  # user is logged in
            return render_template("login.html", username=username,
                                   info="Log into another account here. You will be logged out of your current account.")  # load login page with information
        else:  # if not logged in
            return render_template("login.html",
                                   info="Log in to the website here. Make sure you have an account!")  # load login page with information
...
@app.route("/account/forgot_password",
           methods=["GET", "POST"])  # URL to allow user to change password when they forget it
    ...
    if request.method == "GET":  # user loads page
        ...
        if username != None:  # logged in
            return render_template("forgot_password.html", username=username,
                                   info="If you forgot your password, type in your username so we can send you a link to reset your password. But this is unlikely as you are currently logged in.")
        else:  # not logged in
            return render_template("forgot_password.html", username=username,
                                   info="If you forgot your password, type in your username so we can send you a link to reset your password.")  # render page with info
        ...
        msg.html = '<h1 style="text-align: center;">Hey ' + str(
            ...
            username) + ">here</a>!</h3>" + '</br><h3 style="text-align: center;">If this was not you, kindly ignore it.' '</br><h4 style="text-align: center;">If you have any queries you may send an email to us at adityaswarup241@gmail.com@gmail.com!</h4>'  # code for body of email
...
def resetPassword(userHash):  # reset password function
    if request.method == "GET":  # user loads page
        return render_template("reset_password.html",
                               info="Please enter a new password for your account. ")  # load reset password page
    elif request.method == "POST":  # user submits form
        ...
        if getHashed(username) == userHash:  # if confirmation link matches with username provided
            ...
            if user is None:  # account doesn't exist
                return render_template("reset_password.html", info="Please enter a new password for your account.",
                                       error="Invalid username or password.")  # return with error
            elif password != confirmPassword:  # passwords don't match
                return render_template("reset_password.html", info="Please enter a new password for your account.",
                                       error="Invaalid username or password")
            else:  # everything correct
                user.password = hashedPassword  # change password
                db.session.commit()  # save changes
                return redirect(url_for("home",
                                        success="Your password has been reset! Please proceed to login now."))  # go to homepage with message
    ...
    if request.method == "GET":  # user loads page
        ...
        if username != None:  # user is logged in
            return render_template("forgot_username.html", username=username,
                                   info="If you forgot your username, type in your email so we can send you a list of all your previous usernames. But this is unlikely as you are currently logged in.")
        else:  # not logged in
            return render_template("forgot_username.html", username=username,
                                   info="If you forgot your username, type in your email so we can send you a list of all your previous usernames.")  # render page with info
    elif request.method == "POST":  # user submits form
...
def logout():  # logout function
    ...
    if username != None:  # user logged in
        ...
        return redirect(
            url_for("home", success="You have logged out successfully."))  # redirect to home page with message
    else:  # user not even logged in
        return redirect(url_for("home",
                                success="You have logged out successfully, even though you weren't logged in in the first place."))  # redirect to home page with message
    ...
    if request.method == "GET":  # if page is loaded bu user
        ...
        if username != None:  # user logged in
            return render_template("signup.html", username=username,
                                   info="You can sign up for another account here! You will still be logged into your first account. Note that you need a valid email to create your account.")
        else:  # user not logged in
            return render_template("signup.html", username=username,
                                   info="You can sign up for an account here! Note that you need a valid email to create your account.")
    elif request.method == "POST":  # user submits form
        ...
        if password != "":  # if user keyed in a password
            if password == confirm:  # if both password fields match
                ...
                user = User(username=username, password=hashedPassword, email=email,
                            confirmed="N")  # create new user entry
                ...
                if check is None:  # username is not taken
                    ...
                    return render_template("signup.html",
                                           ...
                                           username=username)  # return to same page with error message
            else:  # passwords do not match
                username = getUsername()  # get username
                return render_template("signup.html", error="Your passwords do not match.",
                                       username=username)  # return to same page with error message
        else:  # no password entered
            username = getUsername()  # get username
            return render_template("signup.html", error="You need to enter a password.",
                                   username=username)  # return to same page with error message
...
def confirmation(userHash):  # confirmation function
    if request.method == "GET":  # user loads page
        return render_template("confirmation.html",
                               info="Please enter your username and password to confirm your account.")  # load confirmation page
    elif request.method == "POST":  # user submits form
        username = request.form["username"]  # get username
        password = request.form["password"]  # get password
        if getHashed(username) == userHash:  # if confirmation link matches with username provided
            ...
            if user is None:  # account doesn't exist
                ...
                return redirect(url_for("home",
                                        success="Your account has been confirmed, though not the first time. Please proceed to login now."))  # go to homepage with message
            else:  # account not confirmed and everything correct
                user.confirmed = "Y"  # change status to confirmed
                db.session.commit()  # save changes
                return redirect(url_for("home",
                                        success="Your account has been confirmed! Please proceed to login now."))  # go to homepage with message
        else:  # username doesn't match
...
def timeline():  # timeline function
    ...
    return render_template("timeline.html", username=username,
                           info="Read about the development of this website here.")  # load timeline with information
...
def contact():  # contact function
    if request.method == "GET":  # user loads the page
        ...
        return render_template("contact.html", username=username,
                               info="Contact the developers through this webpage. You do not have to give your real name.")  # load contact page with info
...
def features():  # function to view features
    ...
    return render_template("features.html", username=username,
                           info="You can understand the features of this website here! Scroll down for more information!")  # render page
...
def events():  # events function
    username = getUsername()  # get username
    return render_template("events.html", username=username, success=request.args.get("success"),
                           info="You can create new events or access and manage previous events from here and discover events. You need to be logged in to create and manage events.")
    ...
    if request.method == "GET":  # user loads page
        ...
        if username != None:  # user logged in
            return render_template("create_event.html", username=username,
                                   info="Create a new event here.")  # render page with information
        else:  # not logged in
            return render_template("create_event.html", username=username,
                                   error="You aren't logged in. Any events you try to create will not be saved.")  # render page with error message
    elif request.method == "POST":  # user submits form
        ...
        if check is None:  # no event with same name
            if username != None:
                ...
                return redirect(url_for("events",
                                        success="Event successfully created! It can be found under find events."))  # return to events page with message
            else:  # user not logged in
                return render_template("create_event.html", username=username,
                                       error="I told you that it would not work.")  # render page with error message
        else:  # name already used
            return render_template("create_event.html", error="Invalid name.",
                                   username=username)  # return and get user to use another name
    ...
    if request.method == "GET":  # user loads page
        ...
        if success is None:  # not redirected
            ...
            if username is None:  # user not signed in
                return render_template("manage_events.html", error=error,
                                       success="Found all events. However, you aren't logged in, so there isn't anything.",
                                       username=username, events=events,
                                       info="View all your events here.")  # return page and message
            elif events is None:  # no events
                return render_template("manage_events.html", error=error,
                                       success="You have not created any events yet, so we can't show anything. Head over to the create events page to create a new one!",
                                       info="View all your events here.")  # return page and message
            else:  # signed in and has events
                return render_template("manage_events.html", error=error, success="Found all of your events!",
                                       username=username, events=events,
                                       info="View all your events here.")  # return page and message
        else:  # redirected
            events = Event.query.filter_by(user=username).all()  # get all events belonging to user
            return render_template("manage_events.html", error=error, success=success, username=username, events=events,
                                   info="View all your events here.")  # return page and message
...
def findEvents():  # function to find events
    if request.method == "GET":  # user loads page
        ...
        error = request.args.get("error")  # error message if redirected from another page
        if success != None:
            return render_template("find_events.html", username=username, events=events, success=success, error=error,
                                   info="Find events here.")  # render page
        else:
            return render_template("find_events.html", username=username, events=events, success="Found all events!",
                                   error=error, info="Find events here.")  # render page
        ...
        if searchType == "user":  # search for user
            ...
            events = Event.query.filter_by(
                name=search).all()  # get events #Please fix the in statement so it works if "He" in "Hello"
        elif searchType == "description":  # search by description
            events = Event.query.filter_by(
                description=search).all()  # get events #Please fix the in statement so it works if "He" in "Hello"
        return render_template("find_events.html", username=username, events=events, info="Find events here.",
                               success="Search sucessful!")  # return with results
        ...
        if event != None:  # event exists
            if username == event.user:  # if user is the creator of the event
                return render_template("edit_event.html", username=username, event=event,
                                       info="Edit details for this event here.")  # load page
            else:  # user did not create this event
                event = Event(id=id, name="HEY!", user="Somebody Else",
                              description="This isn't your event, so don't touch it.")  # Troll event
                return render_template("edit_event.html", username=username, event=event,
                                       info="This form is useless since the event doesn't belong to you.")  # load page
        else:  # no such event
            event = Event(id=id, name="HEY!", user="To be discovered",
                          description="This event doesn't even exist yet. Or it has been deleted.")  # Troll event
            return render_template("edit_event.html", username=username, event=event,
                                   info="This form is useless since the event doesn't exist.")  # load page
        ...
        if event != None:  # event exists
            if event.user == username:  # user is creator of event
                ...
                return redirect(url_for('manageEvents',
                                        success="Event successfully edited."))  # return to manage events with success message
            else:  # not authorised
                return redirect(url_for('manageEvents',
                                        error="You aren't authorised to edit this event"))  # return to manage events with error message
        else:  # no such event
            return redirect(
                url_for('manageEvents', error="No such event."))  # return to manage events with error message
    ...
    if request.method == "GET":  # user loads page
        ...
        if event != None:  # event exists
            if username == event.user:  # user authorised
                return render_template("forms.html", username=username, event=event,
                                       info="Create or manage forms for this event here.",
                                       success="Found your event!")  # render page
            else:  # user not authorised
                event = Event(id=id, name="HEY!", user="Somebody Else",
                              description="This isn't your event, so don't touch it.")  # Troll event
                return render_template("forms.html", username=username, event=event,
                                       info="Create or manage forms for this event here, but nothing will happen since it isn't yours",
                                       success="Found a event!")  # render page
        else:  # no such event
            event = Event(id=id, name="HEY!", user="To be discovered",
                          description="This event doesn't even exist yet. Or it has been deleted.")  # Troll event
            return render_template("forms.html", username=username, event=event,
                                   info="Create or manage forms for this event here, but we can't do anything since the event doesn't exist.",
                                   success="Found a event!")  # render page
        ...
        if event != None:  # if event exists
            if event.user == username:  # if user is authorised
                ...
                if check is None:  # name not used
                    ...
                    return render_template("create_form.html", username=username, error="Invalid name.",
                                           info="Create a new form here.")  # get user to change the name
            return render_template("create_form.html", username=username,
                                   error="You aren't authorised to create a form for this event.",
                                   info="Create a new form here.")  # return and tell user error message
        return render_template("create_form.html", username=username, error="Event does't even exist.",
                               info="Create a new form here.")  # return and tell user error message
    ...
    if request.method == "GET":  # user loads page
        ...
        if event != None:  # event exists
            if username == event.user:  # user is authorised
                return render_template("manage_forms.html", username=username, forms=forms,
                                       success="Found all your forms!",
                                       info="Manage all your forms here.")  # render page with success message
            else:  # not authorised
                forms = [Form(id=id, name="HEY!", user="Somebody Else", event="Unknown",
                              description="This isn't your event, so don't touch it.", questions=[],
                              answers="")]  # troll form
                return render_template("manage_forms.html", username=username, forms=forms, success="Found a form!",
                                       info="Manage all your forms here.")  # render troll page
        else:  # no such event
            forms = [Form(id=id, name="HEY!", user="To be discovered", event="Unknown",
                          description="The event doesn't even exist yet. Or it has been deleted.", questions=[],
                          answers="")]  # troll form
            return render_template("manage_forms.html", username=username, forms=forms, success="Found a form!",
                                   info="Manage all your forms here.")  # render troll page
    ...
    if request.method == "GET":  # user loads page
        ...
        if form != None:  # if form exists
            if username == form.user:  # user authorised
                return render_template("forms_edit.html", username=username, form=form, success="Found your form.",
                                       info="Edit your form here!")  # render page with information
            else:  # user not authorised
                form = Form(id=id, name="HEY!", user="Somebody Else", event="Unknown",
                            description="This isn't your form, so don't touch it.", questions=[],
                            answers="")  # troll form
                return render_template("forms_edit.html", username=username, form=form, success="Found a form.",
                                       info="Edit your form here. There won't be any changes because you aren't authorised.")  # load troll page
        else:  # no such form
            form = Form(id=id, name="HEY!", user="To be discovered", event="Unknown",
                        description="The form doesn't even exist yet. Or it has been deleted.", questions=[],
                        answers="")  # troll form
            return render_template("forms_edit.html", username=username, form=form, success="Found a form.",
                                   info="Edit your form here. But nothing will happen since the form doesn't exist.")  # load troll page
    elif request.method == "POST":  # user submits form
        ...
        if form != None:  # form exists
            if form.user == username:  # user authorised
                ...
                db.session.commit()  # save changes
                return redirect(
                    url_for('manageEvents', success="Form successfully edited."))  # return with success message
            else:  # user not authorised
                return redirect(url_for('manageEvents',
                                        error="You aren't authorised to edit this form."))  # return with error message
        else:  # no such form
        ...
        if form != None:  # form exists
            if username == form.user:  # user is authorised
                ...
                return render_template("forms_questions.html", username=username, form=form, questions=questions,
                                       info="You can edit and add questions to your form here.")  # render page with info
            else:  # user not authorised
                form = Form(id=id, name="HEY!", user="Somebody Else", event="Unknown",
                            description="This isn't your form, so don't touch it.",
                            questions=[[1, "Are you dumb?", "This is not a test.", "radio", ["YES", "NO"]],
                                       [2, "Should you be here?", "Is this your event?", "radio", ["YES", "NO"]]],
                            answers="")  # troll form
                questions = form.questions  # get questions
                return render_template("forms_questions.html", username=username, form=form, questions=questions,
                                       info="You can edit and add questions to your form here.")  # load troll page
        else:  # no such form
            form = Form(id=id, name="HEY!", user="To be discovered", event="Unknown",
                        description="The form doesn't even exist yet. Or it has been deleted.", questions=[],
                        answers="")  # troll form
                ...
                if form.user == username:  # user is authorised
                    ...
                    if options == "":  # no options
                        ...
                        questions.append([(lastID + 1), name, description, type,
                                          ast.literal_eval(options)])  # add question to question list
                    ...
                    questions = form.questions  # get questions
                    return render_template("forms_questions.html", username=username, form=form, questions=questions,
                                           error="You aren't allowed to edit this form.",
                                           info="You can edit and add questions to your form here.")  # load troll page
            else:  # no such form
                form = Form(id=id, name="HEY!", user="To be discovered", event="Unknown",
                            description="The form doesn't even exist yet. Or it has been deleted.", questions=[],
            ...
            if form != None:  # form exists
                if form.user == username:  # user authorised
                    ...
                    with open(form.answers, 'a') as csvFile:  # open CSV file
                        ...
                        csvFile.close()  # close CSV file
                    return redirect(
                        url_for("manageEvents", success="Question successfully edited."))  # return with success message
                else:  # user not authorised
                    form = Form(id=id, name="HEY!", user="Somebody Else", event="Unknown",
                                description="This isn't your form, so don't touch it.",
                                questions=[[1, "Are you dumb?", "This is not a test.", "radio", ["YES", "NO"]],
    ...
    if request.method == "GET":  # user loads page
        ...
        if form != None:  # form exists
            if username == form.user:  # user is authorised
                ...
                return render_template("forms_answers.html", username=username, answers=answers, id=id,
                                       success="Got your form data.",
                                       info="View your form data here.")  # return with data
            else:  # user not authorised
                answers = [["Timestamp", "Are you dumb?", "Should you be here?"],
                           ["A few years ago.", "Yes you are.", "Obviously not."]]  # troll answers
                return render_template("forms_answers.html", username=username, answers=answers, id=id,
                                       success="Got a form's data.",
                                       info="View your form data here. It's pointless as you aren't authorised to see the data.")  # return troll page
        else:  # no such form
            answers = [["Timestamp", "Are you dumb?", "Should you be here?"],
                       ["A few years ago.", "Yes you are.", "Obviously not."]]  # troll answers
            return render_template("forms_answers.html", username=username, answers=answers, id=id,
                                   success="Got a form's data.",
                                   info="View your form data here. There's no data as there isn't a form in the first place.")  # return troll page
    ...
    if request.method == "GET":  # user accesses the download page
        ...
        if form != None:  # if form exists
            if username == form.user:  # user is authorised
                ...
                response.headers['Content-Disposition'] = "attachment; filename=" + str(
                    form.name) + ".csv"  # download csv file
                response.mimetype = 'text/csv'  # type of download
                return response  # return download
            else:  # user not authorised
                csvData = [["Timestamp", "Are you dumb?", "Should you be here?"],
                           ["A few years ago.", "Yes you are.", "Obviously not."]]  # troll data
                answers = []  # empty list for converted data
                answersF = ""  # empty string for converted data
                for i in range(len(csvData)):  # loop over each row in csv data
                    answers.append(",".join(csvData[i]))  # put each row in a list element
...
def viewEvent(id):  # function to view an event
    if request.method == "GET":  # user loads page
        ...
        event = Event.query.filter_by(id=id).first()  # get event
        if event != None:  # event exists
            return render_template("view_event.html", username=username, event=event, success="Found the event.",
                                   info="View details for an event here.")  # render page with message
        else:  # no such event
            return render_template("view_event.html", username=username, event=event,
                                   info="View details for an event here. But there's nothing as the event doesn't exist.")  # render page with message
...
def viewForms(id):  # function to show all forms
    if request.method == "GET":  # user accesses page
        ...
        if forms != None:  # has forms linked to event
            return render_template("view_forms.html", username=username, forms=forms, success="Found all forms.",
                                   info="View all forms for an event here.")  # render page with message
        else:  # if no forms linked to event
            return render_template("view_forms.html",
                                   info="View all forms for an event here. This event does not contain any forms, so there isn't anything.")  # render page with message
...
def fillForm(id):  # function to fill in a form
    if request.method == "GET":  # user loads page
        ...
        if form != None:  # formm exists
            ...
            return render_template("fill_form.html", username=username, form=form, questions=questions,
                                   success="Got form data.",
                                   info="Fill in a form here.")  # render page with questions and messages
        else:  # no such form
            form = Form(id=id, name="HEY!", user="To be discovered", event="Unknown",
                        description="The event doesn't even exist yet. Or it has been deleted.", questions=[],
                        answers="")
            ...
            return render_template("fill_form.html", username=username, form=form, questions=questions,
                                   info="Fill in a form here. There aren't any questions because there isn't a form")  # render page with messages
    elif request.method == "POST":  # user submits form
        form = Form.query.filter_by(id=id).first()  # get form
        if form != None:  # form exists
            ...
            if questions != []:  # form has questions
                ...
                with open(form.answers, 'a') as csvFile:  # open CSV data file
                    ...
                    csvFile.close()  # close CSV file
                return redirect(
                    url_for("findEvents", success="Your response has been recorded."))  # return to find events page
            else:  # form doesn't have questions
                return redirect(url_for("findEvents",
                                        error="Form doesn't have any questions."))  # return to find events page with message
        else:  # form doesn't exist
            return redirect(
                url_for("findEvents", error="Form doesn't exist."))  # return to find events page with message
...
def account():  # function to edit account details
    if request.method == "GET":  # user loads page
        username = getUsername()  # get username
        return render_template("account.html", username=username, success=request.args.get("success"),
                               info="You can edit your account details here.")  # render page with info
...
def changeUsername():  # function to change username
    if request.method == "GET":  # user loads page
        username = getUsername()  # get username
        return render_template("change_username.html", username=username,
                               info="You can change your username here.")  # render page with info
    elif request.method == "POST":  # user submits form
        username = getUsername()  # get username
        ...
        if username == None:  # if not logged in
            return render_template("change_username.html", username=username, error="You are not logged in.",
                                   info="You can change your username here.")  # render page with messages
        else:  # user is logged in
            if username != oldUsername:  # username doesn't match
                return render_template("change_username.html", username=username,
                                       error="Your username does not match with your old username.",
                                       info="You can change your username here.")  # render page with messages
                ...
                if check != None:  # new username already exists
                    return render_template("change_username.html", username=username,
                                           error="The new username is already in use.",
                                           info="You can change your username here.")  # render page with messages
                    ...
                    return redirect(url_for("account",
                                            success="Your username has been successfully changed."))  # return with success message
...
def changeEmail():  # function to change email
    if request.method == "GET":  # user loads page
        ...
        return render_template("change_email.html", username=username,
                               info="You can change your email here.")  # render page with info
    elif request.method == "POST":  # user submits form
        username = getUsername()  # get username
        ...
        user = User.query.filter_by(username=username).first()  # get user details
        if username == None:  # if not logged in
            return render_template("change_email.html", username=username, error="You are not logged in.",
                                   info="You can change your email here.")  # render page with messages
        else:  # user is logged in
            if user.email != oldEmail:  # email doesn't match
                return render_template("change_email.html", username=username,
                                       error="Your email does not match with your old email.",
                                       info="You can change your email here.")  # render page with messagess
            else:  # user is logged in
                user.email = newEmail  # change email
                db.session.commit()  # save changes
                return redirect(url_for("account",
                                        success="Your email has been successfully changed."))  # return with success message
...
def changePassword():  # function to change password
    if request.method == "GET":  # user loads page
        ...
        return render_template("change_password.html", username=username,
                               info="You can change your password here.")  # render page with info here
        ...
        if username != None:  # user is logged in
            if hashedPassword != user.password:  # wrong password
                return render_template("change_password.html", username=username,
                                       ...
                                       info="You can change your password here.")  # return with error message
                ...
                db.session.commit()  # save changes
                return redirect(
                    url_for("account", success="Password successfully changed!"))  # return with success message
        else:  # not logged in
            return render_template("change_password.html", username=username, error="You are not logged in.",
                                   info="You can change your password here.")  # return with error message
...
def deleteAccount():  # function to delete account
    if request.method == "GET":  # user loads page
        ...
        return render_template("delete_account.html", username=username,
                               info="You can delete your account here.")  # render page with info here
    elif request.method == "POST":  # user changes password
        username = getUsername()  # get username
        ...
        if username != None:  # user is logged in
            if hashedPassword != user.password:  # wrong password
                return render_template("delete_account.html", username=username, error="The password is incorrect.",
                                       info="You can change your password here.")  # return with error message
                ...
                session.pop('username', None)  # remove user session
                return redirect(url_for("home",
                                        success="Account successfully deleted! You can restore your account by re-confirming your account again."))  # return with success message
        else:  # not logged in
            return render_template("delete_account.html", username=username, error="You are not logged in.",
                                   info="You can change your password here.")  # return with error message
